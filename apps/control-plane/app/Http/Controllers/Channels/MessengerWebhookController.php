...
use App\Services\MessengerInbound;
 ...
 * GET = hub.challenge handshake against the channel's verify token,
 * POST = X-Hub-Signature-256 HMAC over the raw body with the app secret.
 */
final class MessengerWebhookController extends Controller
{
    ...
    public function receive(
        Request $request,
        Tenancy $tenancy,
        MessengerInbound $inbound,
        string $organizationId,
        string $channelId,
    ): Response {
        $throttle = 'fb-webhook:'.hash('sha256', $channelId);
