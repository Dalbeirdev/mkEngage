...
final class WidgetMessageController extends Controller
{
    ...
    public function store(Request $request, string $conversationId): JsonResponse
    {
        ...
        // Retry with the same key ⇒ original ack, no second row (§27).
        ...
        if ($existing !== null) {
            return response()->json($existing->toContract());
        }

        // Atomic per-conversation sequence: increment under row lock inside
        // the request transaction (RULES-message-ordering #1).
        ...
        $message = Message::query()->create([
            ...
            'sender_type' => 'visitor',
            'sender_id' => $this->visitor($request)->id,
        ...
        return response()->json($message->toContract(), 201);
    }
